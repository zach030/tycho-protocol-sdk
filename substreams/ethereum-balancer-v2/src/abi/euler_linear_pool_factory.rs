...
pub mod functions {
    ...
    pub struct Create {
        pub name: String,
        pub symbol: String,
        pub main_token: Vec<u8>,
        pub wrapped_token: Vec<u8>,
        pub upper_target: substreams::scalar::BigInt,
        pub swap_fee_percentage: substreams::scalar::BigInt,
        pub owner: Vec<u8>,
        pub protocol_id: substreams::scalar::BigInt,
    }
    impl Create {
        const METHOD_ID: [u8; 4] = [223u8, 138u8, 249u8, 137u8];
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[
                ...
                ethabi::Token::Address(ethabi::Address::from_slice(&self.main_token)),
                ethabi::Token::Address(ethabi::Address::from_slice(&self.wrapped_token)),
                ethabi::Token::Uint(ethabi::Uint::from_big_endian(
                    match self.upper_target.clone().to_bytes_be() {
                ...
                ethabi::Token::Uint(ethabi::Uint::from_big_endian(
                    match self.protocol_id.clone().to_bytes_be() {
                        (num_bigint::Sign::Plus, bytes) => bytes,
                        (num_bigint::Sign::NoSign, bytes) => bytes,
                        (num_bigint::Sign::Minus, _) => {
                            panic!("negative numbers are not supported")
    ...
    #[derive(Debug, Clone, PartialEq)]
    pub struct EulerProtocol {}
    impl EulerProtocol {
        const METHOD_ID: [u8; 4] = [82u8, 8u8, 252u8, 43u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            Ok(Self {})
        }
        pub fn encode(&self) -> Vec<u8> {
    ...
    impl substreams_ethereum::Function for EulerProtocol {
        const NAME: &'static str = "eulerProtocol";
        fn match_call(call: &substreams_ethereum::pb::eth::v2::Call) -> bool {
            Self::match_call(call)
        }
        fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
    ...
    impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for EulerProtocol {
        fn output(data: &[u8]) -> Result<Vec<u8>, String> {
            Self::output(data)
        }
    }
    ...
    #[derive(Debug, Clone, PartialEq)]
    pub struct GetLastCreatedPool {}
    impl GetLastCreatedPool {
        const METHOD_ID: [u8; 4] = [94u8, 211u8, 18u8, 130u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            Ok(Self {})
        }
        pub fn encode(&self) -> Vec<u8> {
    ...
    impl substreams_ethereum::Function for GetLastCreatedPool {
        const NAME: &'static str = "getLastCreatedPool";
        fn match_call(call: &substreams_ethereum::pb::eth::v2::Call) -> bool {
            Self::match_call(call)
        }
        fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
    ...
    impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for GetLastCreatedPool {
        fn output(data: &[u8]) -> Result<Vec<u8>, String> {
            Self::output(data)
        }
    }
...
pub mod events {
    ...
    impl EulerLinearPoolCreated {
        const TOPIC_ID: [u8; 32] = [
            222u8, 57u8, 57u8, 206u8, 202u8, 16u8, 20u8, 97u8, 253u8, 126u8, 56u8, 151u8, 5u8, 2u8,
            10u8, 31u8, 139u8, 21u8, 24u8, 237u8, 183u8, 93u8, 5u8, 40u8, 75u8, 33u8, 170u8, 39u8,
            3u8, 84u8, 195u8, 220u8,
        ];
            ...
            Ok(Self {
                ...
                protocol_id: {
                    ...
                    ethabi::decode(
                            ...
                            .map_err(|e| {
                                format!(
                                    "unable to decode param 'protocol_id' from topic of type 'uint256': {:?}",
                                    e
                                )
                            })?
                            .pop()
    ...
    impl substreams_ethereum::Event for EulerLinearPoolCreated {
        const NAME: &'static str = "EulerLinearPoolCreated";
