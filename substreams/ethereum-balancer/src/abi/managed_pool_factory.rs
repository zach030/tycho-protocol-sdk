            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ...
                        ethabi::Token::Tuple(
                            vec![
                                { let v = self.settings_params.0.iter().map(| inner |
                                ethabi::Token::Address(ethabi::Address::from_slice(&
                                inner))).collect(); ethabi::Token::Array(v) }, { let v =
                                self.settings_params.1.iter().map(| inner |
                                ...
                                .settings_params.2.clone().to_bytes_be() {
                                (num_bigint::Sign::Plus, bytes) => bytes,
                                (num_bigint::Sign::NoSign, bytes) => bytes,
                                (num_bigint::Sign::Minus, _) => {
                                panic!("negative numbers are not supported") }, }
                                .as_slice(),),), ethabi::Token::Bool(self.settings_params.3
                                .clone()), ethabi::Token::Bool(self.settings_params.4
                                .clone()),
                                ethabi::Token::Uint(ethabi::Uint::from_big_endian(match self
                                .settings_params.5.clone().to_bytes_be() {
                                (num_bigint::Sign::Plus, bytes) => bytes,
                                (num_bigint::Sign::NoSign, bytes) => bytes,
                                (num_bigint::Sign::Minus, _) => {
                                panic!("negative numbers are not supported") }, }
                                .as_slice(),),),
                                ethabi::Token::Uint(ethabi::Uint::from_big_endian(match self
                                .settings_params.6.clone().to_bytes_be() {
                                (num_bigint::Sign::Plus, bytes) => bytes,
                                (num_bigint::Sign::NoSign, bytes) => bytes,
                                (num_bigint::Sign::Minus, _) => {
                                panic!("negative numbers are not supported") }, }
