...
pub mod functions {
    ...
    #[derive(Debug, Clone, PartialEq)]
    pub struct InitialiseOwnership {
        pub fee_receiver: Vec<u8>,
        pub admin: Vec<u8>,
    }
    impl InitialiseOwnership {
        const METHOD_ID: [u8; 4] = [69u8, 230u8, 47u8, 133u8];
        pub fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
            let maybe_data = call.input.get(4..);
            if maybe_data.is_none() {
                return Err("no data to decode".to_string());
    ...
    impl substreams_ethereum::Function for InitialiseOwnership {
        const NAME: &'static str = "initialise_ownership";
        fn match_call(call: &substreams_ethereum::pb::eth::v2::Call) -> bool {
            Self::match_call(call)
        }
        fn decode(call: &substreams_ethereum::pb::eth::v2::Call) -> Result<Self, String> {
...
pub mod events {
    ...
    #[derive(Debug, Clone, PartialEq)]
    pub struct TwocryptoPoolDeployed {
        pub pool: Vec<u8>,
        pub name: String,
        pub symbol: String,
        pub coins: [Vec<u8>; 2usize],
        pub math: Vec<u8>,
        pub salt: [u8; 32usize],
        pub precisions: [substreams::scalar::BigInt; 2usize],
    ...
    impl TwocryptoPoolDeployed {
        const TOPIC_ID: [u8; 32] = [
            129u8, 82u8, 163u8, 3u8, 126u8, 61u8, 197u8, 65u8, 84u8, 173u8, 13u8, 44u8, 173u8,
            177u8, 207u8, 126u8, 29u8, 27u8, 158u8, 43u8, 98u8, 95u8, 170u8, 61u8, 251u8, 79u8,
            224u8, 61u8, 96u8, 145u8, 2u8, 202u8,
        ];
        pub fn match_log(log: &substreams_ethereum::pb::eth::v2::Log) -> bool {
            if log.topics.len() != 1usize {
                return false;
            }
            if log.data.len() < 512usize {
                return false;
            }
            return log
                .topics
    ...
    impl substreams_ethereum::Event for TwocryptoPoolDeployed {
        const NAME: &'static str = "TwocryptoPoolDeployed";
        fn match_log(log: &substreams_ethereum::pb::eth::v2::Log) -> bool {
            Self::match_log(log)
        }
        fn decode(log: &substreams_ethereum::pb::eth::v2::Log) -> Result<Self, String> {
